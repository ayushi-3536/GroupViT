...
class MultiLabelContrastive_PACL(nn.Module):
    ...
    def encode_image(self, image, text_token=None, *, return_feat=False, as_dict=False, cross_attn=True):
        outs = Result(as_dict)
        ...
        if cross_attn:
            image_x = self.img_projector(img_outs['x'])
            assert text_token is not None
            #For single text
            if text_token.ndim == 2:
                text_token = text_token.unsqueeze(1)
            logit_scale = torch.clamp(self.logit_scale.exp(), max=100)
            sim = (image_x @ text_token.transpose(-2, -1)) * logit_scale
            attn = F.softmax(sim, dim=-2)
            #For multi-label texts
            if text_token.ndim == 3:
                attn = torch.mean(attn, dim=-1, keepdim=True)
            weighted_embeddings = image_x.transpose(-1, -2) @ attn
            image_x = weighted_embeddings.squeeze(-1)
        else:
            image_x = self.img_projector(self.img_encoder.forward_image_head(img_outs['x']) )    

        outs.append(image_x, 'image_x')
