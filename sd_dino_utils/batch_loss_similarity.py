...
print("prob tensor dtype", prob_tensor.dtype)
#save the tensor in a csv file
np.savetxt("/misc/student/sharmaa/groupvit/GroupViT/saved_soft_attn.csv", prob_tensor, delimiter=",")

#replicate the tensor 256 time to change the shape from [64, 196] to [256, 64, 196]
prob_tensor = prob_tensor.repeat(256, 1, 1)
#print("after reshape prob_tensor.shape", prob_tensor.shape)
# Get the indices of the maximum values along the last dimension
max_indices = torch.argmax(prob_tensor, dim=1)
print("max_indices.shape", max_indices.shape)
print("max_indices", max_indices)

# Convert the indices to one-hot vectors
onehot_prob_tensor =  rearrange(F.one_hot(max_indices, num_classes=prob_tensor.shape[1]).to(dtype=prob_tensor.dtype), 'b n d -> b d n')
#onehot_prob_tensor =  F.one_hot(max_indices, num_classes=prob_tensor.shape[1]).to(dtype=prob_tensor.dtype)

# Print the one-hot encoded matrix
# print(onehot_prob_tensor.shape)
# print(prob_tensor.shape)

updated_prob = prob_tensor * onehot_prob_tensor
...
#replicate the tensor 256 time to change the shape from [196, 196] to [256, 196, 196]
print("dtype of dino_dist_mat", dino_dist_mat.dtype)    
dino_dist_mat = torch.tensor(dino_dist_mat).repeat(256, 1, 1)
print("after reshape dino_dist_mat.shape", dino_dist_mat.shape)

# Assuming dino_dist_mat is a tensor of shape [b, n, d]
b, n, d = dino_dist_mat.shape

# Create a boolean mask to select elements based on the condition
print("checking max indices", max_indices[:, None, :].shape )
print("checking max indices", max_indices[:, :, None].shape )
